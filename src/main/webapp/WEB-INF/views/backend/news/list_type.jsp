...
<aside class="right-side">
	...
	<section class="content-header">
		<h3>
			新闻类型列表 <small><a class="btn btn-info btn-sm" href="/admin/news/addType.htm">添加类型</a> </small>
		</h3>
	</section>
	<!-- Main content -->
	<section class="content">
		<form id="searchForm" action="" method="get" class="form-inline">
			<div class="input-group">
				<input type="text" name="search" id="search" placeholder="名称" value="<%=request.getParameter("search") == null ? "" : request.getParameter("search") %>" class="form-control input-sm"> <span class="input-group-btn">
				...
				<table width="100%" class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<td><%=rows.get(i).get("name")%></td>
							<td><%=rows.get(i).get("addtime")%></td>
							<td><%=rows.get(i).get("edittime")%></td>
							<td><a href="addType.htm?action=edit&id=<%=rows.get(i).get("id")%>&url=<%=request.getAttribute("url")%>">修改</a></td>
						</tr>
						<%
							}
						              }
